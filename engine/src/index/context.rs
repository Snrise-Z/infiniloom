//! Context expander for diff-based queries.
//!
...
use super::types::{DepGraph, FileEntry, IndexSymbol, IndexSymbolKind, SymbolIndex};
...
impl<'a> ContextExpander<'a> {
    ...
    pub fn classify_change(
        ...
        if let Some(diff) = &change.diff_content {
            ...
            let doc_indicators = ["///", "//!", "/**", "/*", "#", "\"\"\"", "'''"];
            let all_doc_changes = diff.lines()
                .filter(|l| l.starts_with('+') || l.starts_with('-'))
                .filter(|l| l.len() > 1) // Skip empty diff markers
                .all(|line| {
                    let trimmed = line[1..].trim();
                    trimmed.is_empty() || doc_indicators.iter().any(|ind| trimmed.starts_with(ind))
                });
            if all_doc_changes {
    ...
    fn classification_score_multiplier(&self, classification: ChangeClassification) -> f32 {
...
mod tests {
    ...
    use crate::index::types::{
        FileEntry, FileId, IndexSymbolKind, Language, Span, SymbolId, Visibility,
    };
